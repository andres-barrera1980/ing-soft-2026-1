...
El código original era un completo desastre porque violaba el Principio de Responsabilidad Única. Una clase debe tener una, y solo una, razón para cambiar, pero GestorLibro tenía seis razones distintas para cambiar: validaba datos, se conectaba a la base de datos a pedal con JDBC puro, generaba el slug para la URL, mandaba correos, logueaba en consola e indexaba en Elasticsearch.  A esto se le sumaban los típicos pecados de diseño que vimos en clase. Primero, el acoplamiento rígido, ya que la clase creaba sus propias instancias de EmailService y SearchIndex adentro del método. Al no usar Inyección de Dependencias, era imposible meterle mocks de Mockito, dejando la testabilidad en cero. Segundo, violaba el Principio de Inversión de Dependencias (DIP) porque el módulo de alto nivel dependía directamente de detalles de bajo nivel en lugar de abstracciones. Y tercero, tenía las credenciales hardcoded, quemando las contraseñas y URLs en el código, lo que obligaría a recompilar el código de negocio si cambian el servidor de correo en producción.  Para arreglarlo y separar las fronteras del sistema. Se organizo el código bajo el enfoque de Puertos y Adaptadores y los principios SOLID. Se dividio todo en tres capas limpias. En la capa de dominio y aplicación esta el servicio GestorLibro, que ahora tiene la única responsabilidad de orquestar el flujo, la entidad Libro y una clase utilitaria pura llamada SlugUtils para las URLs. Luego se definio los Puertos, que son interfaces del dominio como LibroRepository, NotificacionService e IndiceLibroService, haciendo que el núcleo del negocio solo conozca estos contratos para cumplir con DIP y OCP. Finalmente, en los Adaptadores de infraestructura moví los detalles tecnológicos hacia afuera, usando Spring Data JPA para la persistencia, JavaMailSender para el correo y un cliente HTTP para Elasticsearch. Además, se saco todas las credenciales del código hacia el archivo application.properties usando @Value de Spring.  Al final, gracias a la inyección por constructor que gestiona Spring, el método principal en GestorLibro quedó supercorto y limpio: primero valida los datos de forma interna, luego delega la creación del slug a la clase utilitaria de dominio, y después simplemente delega la persistencia, la notificación y la indexación a sus respectivos servicios abstractos. La prueba de que este refactoring funciona es que ahora tenemos una testabilidad unitaria real. Gracias a que dependemos de interfaces, se puede probar la lógica de negocio de forma aislada, sin necesidad de levantar bases de datos reales ni servidores de correo, pudiendo verificar con verifyNoInteractions() que no se toque la infraestructura si la validación llega a fallar. Con este diseño el sistema queda robusto, mantenible y listo para escalar.  
